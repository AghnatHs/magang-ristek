...
<body>
    ...
    <div class="container mt-4">
        <a class="btn btn-outline-primary" href="/komnews">All</a>
        @foreach ($categories as $category)
            <a class="btn {{ $category->name == $cat ? 'btn btn-danger' : 'btn btn-outline-primary' }}"
                href="/komnews/category/{{ $category->slug }}" role="button">{{ $category->name }}</a>
        @endforeach
        <h1>{{ $title }}</h1>
        <h1>-------------</h1>
        @foreach ($komnews as $item)
            <h2>{{ $item->title }}</h1>
                <p>{{ $item->content }}</p>
